...
class ReconnectingNodeConsumer(object):
    ...
    def run(self):        
        ...
        for dch in defaultnodes_hosts:
            try:
                self._defaultnodes.append(socket.gethostbyname(dch))
                logmsg='Defaultnodes obtained: ' + ' '.join(self._defaultnodes)
                LOGGER.debug(logmsg)
            except:
    ...
    def _update_nodeslist(self):
        #Reinit list
        self._nodeslist=[]
        #Get list of nodes through http requests on localhost (it includes itself if being a seed node)
        try:
                    ...
                    if 'cluster_links' in r[j]:
                        for k in range(len(r[j]['cluster_links'])):
                            if 'peer_addr' in r[j]['cluster_links'][k]:
                                if r[j]['cluster_links'][k]['peer_addr'] not in self._nodeslist:
                                    self._nodeslist.append(r[j]['cluster_links'][k]['peer_addr'])
        except:
